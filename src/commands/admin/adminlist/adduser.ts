...
module.exports = {
  data:
    new SlashCommandBuilder()
      .setName('promotetoadmin')
      .setDefaultMemberPermissions(0)
      .setDescription('Adds a user to the adminlist.')
      .addMentionableOption(option =>
        option.setName('user')
          .setRequired(true)
          .setDescription('The User to add to the adminlist'))
  ...
  async execute(interaction: Interaction) {
    ...
    if (await adminlistService.exists(user)) {
      await interaction.reply({ content: `${mentionable} is already adminlist, try update instead.`, ephemeral: true });
    } else {
